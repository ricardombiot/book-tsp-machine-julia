module SolutionGraphReader
    ...
    function get_graph_join_origin(machine :: IMachine) :: Union{Graph, Nothing}
        ...
        action_id = DatabaseInterface.generate_action_id(db, actual_km, color_origin)

        graph_join = nothing

        total_graphs_joined = 0
        for (parent_id, graph_solution) in get_all_solution_graph(machine)
            ...
            if copy_graph_solution.valid
                ...
                total_graphs_joined += 1
            end
        end

        #println("total_graphs_joined = $total_graphs_joined ")
        return graph_join
    end
