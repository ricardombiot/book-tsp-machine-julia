...
function delete_edge_by_id!(graph :: Graph, edge_id :: EdgeId)
    exist_origin_and_destine = have_node(graph, edge_id.origin_id) && have_node(graph, edge_id.destine_id)
    exist_edge = have_edge(graph, edge_id.origin_id, edge_id.destine_id)
    if exist_origin_and_destine && exist_edge
