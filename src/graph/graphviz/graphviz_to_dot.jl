...
function action_id_to_dot(graph :: Graph, action_id :: ActionId) :: String
    ...
    for (NodeId, node) in graph.table_nodes[action_id]
        ...
        fillcolor = ""
        #fillcolor = paint_fillcolor(color)
        node_definition = "$name [$fillcolor  label=$node_label_html]"
...
function paint_fillcolor(color :: Color) :: String
    list = [
            "#9ef3c6", "#f3d69e", "#d5f5b1", "#fbf796", "#83d0f0",
            "#e37c7c", "#79ada4", "#adad9c", "#efc6f2", "#8388c9"
           ]

    if length(list) < color+1
        hex_color = "#ffffff"
    else
        hex_color = list[color+1]
    end

    fillcolor = "fillcolor=\"$(hex_color)\""
    return fillcolor
end
